...
/*!
    \brief      channel commutation control shadow register enable
    \param[in]  timer_periph: TIMERx(x=0,7)
    \param[in]  newvalue: ENABLE or DISABLE 
    \param[out] none
    \retval     none
...
/*!
    \brief      configure commutation control shadow register update control
    \param[in]  timer_periph: TIMERx(x=0,7)
...
/*!
    ...
    \param[in]  ocmode: channel output compare mode
                only one parameter can be selected which is shown as below:
      \arg        TIMER_OC_MODE_TIMING: frozen mode
      \arg        TIMER_OC_MODE_ACTIVE: set the channel output
      \arg        TIMER_OC_MODE_INACTIVE: clear the channel output
      \arg        TIMER_OC_MODE_TOGGLE: toggle on match
